        ...
        <div v-if="showDetailModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" @click="closeDetailModal">
            <div class="bg-white rounded-2xl w-full max-w-4xl overflow-hidden shadow-2xl relative" @click.stop>
                ...
                <div class="p-8">
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-100 flex justify-end" v-if="selectedLeave && !['REJECTED', 'CANCELLED'].includes(selectedLeave.status)">
                        <button 
                            @click="cancelLeave(selectedLeave)" 
                            class="bg-red-50 border border-red-500 text-red-600 font-bold py-2.5 px-8 rounded-lg hover:bg-red-500 hover:text-white transition-colors duration-200">
                            Batalkan Pengajuan
                        </button>
                    </div>
